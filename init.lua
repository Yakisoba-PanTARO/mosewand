...
minetest.register_craftitem("mosewand:mosewand", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if above_node.name == "mosewand:air" then
			local pos_stack = {}
			table.insert(pos_stack, pointed_thing.above)

			while #pos_stack ~= 0 do
				local pos = table.remove(pos_stack)
				if minetest.get_node(pos).name == "mosewand:air" then
					minetest.remove_node(pos)
					for _, dir in ipairs(directions) do
						table.insert(pos_stack, vector.add(pos, dir))
					end
				end
			end
		...
		elseif minetest.get_item_group(under_node.name, "liquid") > 0 then
			...
			local function place_dummy_nodes(center_pos)
				local count = 0

				local function place_dummy_nodes_vertically(pos)
				...
				if count > 0 then
					...
					return place_dummy_nodes(vector.round(pos2))
				end
			end
